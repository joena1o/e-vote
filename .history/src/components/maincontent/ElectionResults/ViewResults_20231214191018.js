...
const ViewResults = () => {
  ...
  // Pagination
  const itemsPerPage = isLargerThan768 ? 4 : 2;
  ...
  const cardWidth = isLargerThan768 ? 'calc(25% - 16px)' : '100%';
  const cardHeight = isLargerThan768 ? '300px' : 'auto';

  return (
    <Box p={4} bg="white" pb="40px">
      ...
      <Wrap spacing={4} justify="center">
        {currentResults.map((election) => (
          <WrapItem key={election.id} w={cardWidth}>
