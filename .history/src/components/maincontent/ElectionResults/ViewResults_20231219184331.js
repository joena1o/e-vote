...
const ViewResults = () => {
    ...
    if (Array.isArray(mockData)) {
      ...
      if (filterElectionType) {
        filteredResults = filteredResults.filter(
          (election) => election.electionType === filterElectionType
        );
      }
  ...
  }, [sortOrder, filterElectionType, filterDate]);
      ...
      <Flex justifyContent="space-between" alignItems="center" mb={4} direction={isLargerThan768 ? 'row' : 'column'}>
        ...
        <Box mb={isLargerThan768 ? 0 : 4} mr={isLargerThan768 ? 4 : 0}>
          <Select
            ...
            onChange={(selectedOption) =>
              setFilterDate(selectedOption ? new Date(`${selectedOption.value}-01-01`) : null)
            }
          />
        </Box>
        <Button
