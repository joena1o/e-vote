...
const ManageStudents = () => {
  ...
  const [selectedFacultyFilter, setSelectedFacultyFilter] = useState('All'); // Initial faculty filter value
  const [filters, setFilters] = useState({}); // State for input field filters
    ...
    Object.keys(filters).forEach((key) => {
      if (filters[key]) {
        result = result.filter((student) =>
          student[key].toLowerCase().includes(filters[key].toLowerCase())
        );
      }
    });
    ...
    <ChakraProvider>
      <Box>
        <HStack spacing={2} align="center" w="100">
          ...
          {Object.keys(mockStudents[0]).map((key) => (
            <Input
              key={key}
              type="text"
