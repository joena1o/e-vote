...
const GlobalStyle = createGlobalStyle`
  body {
    ...
    color: ${props => (props.theme === 'dark' ? '#fff' : '#000')}; // Set other component texts to black in light mode
  }
`;

const Header = styled.header`
  background: ${props => (props.theme === 'dark' ? '#333' : '#fff')};
  color: ${props => (props.theme === 'dark' ? '#fff' : '#ce7348')}; // Set navbar text to brown in light mode
  padding: 10px 20px;
  display: flex;
  align-items: center;
