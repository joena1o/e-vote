...
const GlobalStyle = createGlobalStyle`
  body {
    ...
    color: ${props => (props.theme === 'dark' ? '#fff' : '#000')}; // Set text color to black in light mode
  }
`;

const Header = styled.header`
  background: ${props => (props.theme === 'dark' ? '#333' : '#fff')};
  color: ${props => (props.theme === 'dark' ? '#fff' : '#000')}; // Set header text color to black in light mode
  padding: 10px 20px;
  display: flex;
  align-items: center;
