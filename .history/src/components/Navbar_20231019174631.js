...
const GlobalStyle = createGlobalStyle`
  body {
    background-color: ${props => (props.theme === 'dark' ? '#333' : '#fff')}; // Changed back to #333 for dark mode
    color: ${props => (props.theme === 'dark' ? '#fff' : '#ce7348')};
  }
`;

const Header = styled.header`
  background: ${props => (props.theme === 'dark' ? '#000' : '#fff')}; // Navbar background is black in dark mode
  color: ${props => (props.theme === 'dark' ? '#fff' : '#ce7348')};
  padding: 10px 20px;
  display: flex;
  align-items: center;
...
const Slider = styled.div`
  width: 40px;
  height: 20px;
  background-color: ${props => (props.theme === 'dark' ? '#333' : '#f4f4f4')}; // Changed the dark mode background color to black (#000) for the slider
  border-radius: 20px;
  position: relative;
